	...
	/*	Console
			...
			.assert(boolean)
			.count()	incrémente une valeur à partir de 1
			.time(name) lance un chronomètre, l'arrêter avec .timeEnd(name) pour obtenir le résultat.
...
dessinerJeu = function () {
	...
	// Pour chaque colonne
	for (let i = départx; i < finx; i++)
		// Pour chaque ligne
		for (let j = départY; j < finY; j++)
...
lecturePause = function () {
	...
	enCours = !enCours
	boutonLecturePause.innerText = enCours ? "Pause" : "Jouer" // "⏸" : "⯈"
	enCours && actualiserJeu()
}

changerFréquence = function () {
	// Change la valeur du nombre de générations affichées par seconde
	objetParamètres.générationsParSeconde = entréeGénérationsParSeconde.value

	// Il ne peut pas être inférieur à min ni supérieur à max
	if (entréeGénérationsParSeconde.value > (entréeGénérationsParSeconde.getAttribute("max") - 0))
		entréeGénérationsParSeconde.value = entréeGénérationsParSeconde.getAttribute("max")

	else if (entréeGénérationsParSeconde.value < (entréeGénérationsParSeconde.getAttribute("min") - 0))
		entréeGénérationsParSeconde.value = entréeGénérationsParSeconde.getAttribute("min")
